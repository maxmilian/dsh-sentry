...
- 一组对目标组织具备读取权限的 Sentry **User Auth Token**（`sntryu_`）
...
| `token` | `SENTRY_AUTH_TOKEN` | 必填 | User auth token（`sntryu_`）。Organization auth token 只有 `org:ci`，会被返回 403。永不返回、永不写入日志。 |
